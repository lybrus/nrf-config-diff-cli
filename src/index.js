...
const appSettingsFile = 'app_settings.h'
...
const appSettingsPath = path.join(cwd, appSettingsFile)
...
;(async () => {
    ...
    if (fs.existsSync(appConfigPath)) {
        const { override } = await inquirer.prompt([{
            type: 'confirm',
            name: 'override',
            message: `${appConfigFile} is exists, do you want to override it?`,
            default: false
        }])
...
`, 'utf-8')
    ...
    if (unknown.length) {
        ...
        fs.writeFileSync(appSettingsPath, `/* Generated by nrf-config-diff-cli */
